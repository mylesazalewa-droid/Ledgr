...
export default function AddItemModal({ onClose }) {
  ...
  // ─── Step content ─────────────────────────────────────────────────────────
  // Rendered the same way on mobile and desktop; the wrapper differs.
  ...
  return (
    ...
    >
      {isMobile ? (
        /*
         * MOBILE — plain <div> with a CSS @keyframes slide-up.
         *
         * Why not motion.div here?
         * Framer Motion's spring leaves a live `transform: translateY(Xpx)` on
         * the element that keeps being updated until the spring fully settles.
         * On iOS Safari, any JS-driven transform on an ancestor element causes
         * child touch hit-tests to be silently dropped — buttons become
         * completely unresponsive. CSS @keyframes animations end cleanly and
         * leave NO residual JS transform, so touch events work correctly.
         */
        <div
        ...
        >
          {Header}
          {Progress}
          {/* On mobile: no AnimatePresence/motion.div step transitions.
              Those leave transform:translateX(0px) on an overflow:auto ancestor,
              which triggers the same iOS touch bug. Direct conditional render is fine. */}
        ...
        /*
         * DESKTOP — Framer Motion scale animation is safe; no iOS touch issues.
         */
        <motion.div
